...
export async function POST(req: NextRequest) {
  try {
    ...
    // Embed thumbnail images — crop to extract ONLY the colored reference
    // (top-left 86×86px area of each page) and upscale for quality.
    // Thumbnails are 280×362px (scaled from 612×792pt page).
    // Colored ref at (36,36) size 86×86 in original → in thumbnail: ~16.5px, ~16.5px, ~39px
      ...
      for (const url of thumbnailUrls.slice(0, 6)) {
        try {
          ...
          if (imgRes.ok) {
            ...
            // Crop the colored reference from top-left of the thumbnail
            // Thumbnail is 280×362, colored ref is at ~(14,14) with size ~44
            const croppedBuf = await sharp(imgBuf)
              .extract({ left: 13, top: 13, width: 46, height: 46 })
            ...
            let embedded;
            try {
              embedded = await doc.embedPng(croppedBuf);
            } catch {
              embedded = await doc.embedJpg(croppedBuf);
            }
